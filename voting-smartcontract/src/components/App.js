...
import NewVotingContract  from '../abis/NewVotingContract.json';
...
class App extends Component {
    ...
    async loadWeb3() {
        if(window.ethereum) {
            ...
            console.log('entra al 2 ', window.web3.currentProvider);
    ...
    async loadBlockchainData() {
        ...
        console.log(this.state.vContract.methods);
        const activeStack = this.state.vContract.methods.activeStack.call();
        this.setState({ activeStack });

        this.setState({ loading: false });

       
        // Load TokenFarming 
        console.log('Load TokenFarming');
        const VotingContractData = NewVotingContract.networks[networkID];
        if(VotingContractData) {
            // Get web3 version of funcToken SmartContract
            const vContract = await new web3.eth.Contract(
                NewVotingContract.abi, NewVotingContract.address
            )
