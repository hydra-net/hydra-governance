...
function App() {
  ...
  const initTokenContract = async () => {
    ...
    const eToken = new web3.eth.Contract(eVotingTokenContract.abi, eVotingTokenAddress);
  ...
  const calculateWinners = async() => {
    return new Promise(async(resolve, reject) => {
      try {
        ...
        const accountData = await getCurrentAccount();
        const { account, web3 } = accountData;
        const resultWinners = await contract.methods.reviewWinningProposals(account).send({from: account});
